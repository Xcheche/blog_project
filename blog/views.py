...
from django.contrib.auth.mixins import (
    LoginRequiredMixin,
    UserPassesTestMixin,
)  # (UserPassesTextMixin -- used for restricting unathourized people)
...
class PostCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(
        self, form
    ):  # Django cbv view without form dosent require save=false
    ...
    def get_success_url(self):
        ...
        )  # Use the post's absolute URL (the detail page URL)
...
class PostDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def test_func(
        self,
    ):  # Check if the current user is the author of the post before they can delete
        post = self.get_object()
        if self.request.user == post.author:
            return True
        return False
